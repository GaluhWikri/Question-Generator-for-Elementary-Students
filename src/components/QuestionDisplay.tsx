...
import { Download, RefreshCw, BookOpen, User, BarChart3, Eye, EyeOff, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      <div className="p-6 bg-slate-800/50 rounded-xl shadow-lg">
        <h2 className="text-2xl font-bold mb-6 text-purple-300 border-b-2 border-purple-500/30 pb-3">Ringkasan Soal</h2>
        
        {/* Tampilan Baru untuk Mata Pelajaran dan Kelas */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 text-center">
          <div className="bg-slate-700/50 p-4 rounded-lg flex flex-col items-center justify-center">
            <BookOpen className="w-6 h-6 mb-2 text-blue-400"/>
            <span className="text-sm text-gray-400">Mata Pelajaran</span>
            <span className="font-semibold text-lg">{subject}</span>
          </div>
          <div className="bg-slate-700/50 p-4 rounded-lg flex flex-col items-center justify-center">
            <User className="w-6 h-6 mb-2 text-green-400"/>
          ...
          <div className="bg-slate-700/50 p-4 rounded-lg flex flex-col items-center justify-center">
            ...
            <span className="font-semibold text-lg capitalize">{requestedDifficulty || 'Standar'}</span>
          </div>
        </div>

        <div className="flex flex-col sm:flex-row gap-4 mt-6">
          <button onClick={onRegenerateQuestions} className="w-full sm:w-auto flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2">
            <RefreshCw className="w-4 h-4" /> Buat Ulang
          </button>
          <button onClick={handleDownloadPDF} className="w-full sm:w-auto flex-1 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2">
            <Download className="w-4 h-4" /> Unduh PDF
          </button>
          <button onClick={() => setShowAnswers(!showAnswers)} className="w-full sm:w-auto flex-1 px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2">
      ...
      {questions.map((q, index) => (
        <div key={index} className="p-6 bg-slate-800/50 rounded-xl shadow-lg transition-all hover:bg-slate-800/80">
          ...
          {q.type === 'multiple-choice' && q.options && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              {q.options.map((option, i) => (
                <div key={i} className={`p-3 rounded-md border transition-all flex items-center justify-between ${showAnswers && i === q.correctAnswer ? 'bg-green-500/20 border-green-500 text-white' : 'bg-slate-700/50 border-slate-600 text-gray-300'}`}>
                  <div>
          ...
          {showAnswers && q.type === 'fill-in-the-blank' && (
            <div className="mt-2">
              <p className="text-sm text-gray-400">Kunci Jawaban:</p>
              <p className="p-3 rounded-md bg-green-500/20 border border-green-500 text-white italic">{q.correctAnswer}</p>
            </div>
          )}
          {showAnswers && q.type === 'essay' && (
            <div className="mt-2">
              <p className="text-sm text-gray-400">Panduan Jawaban:</p>
              <p className="p-3 rounded-md bg-green-500/20 border border-green-500 text-gray-300 text-sm">{q.correctAnswer}</p>
            </div>
          )}
        </div>
      ))}
